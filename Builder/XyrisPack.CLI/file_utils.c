...
char* create_stub_key(const char* original_path, const char* key, int startup, int unhook, int antidump, int delay) {
    ...
    if (!find_and_replace(hFile, key, startup == 1, unhook == 1, antidump == 1, delay == 1)) {
        printf("Failed to find or replace required strings in .data section\n");
        CloseHandle(hFile);
